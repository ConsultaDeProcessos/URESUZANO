...
// Versão 11.6 Card Elevation — Destaque e Contraste
// ============================================================
...
// --- MOTOR DE HUMANIZAÇÃO CONSULTIVA (v11.6) ---
function gerarMensagemHumanizada(processo) {
    ...
    // SITUAÇÃO: NÃO FAZ JUS (Embasamento Legal LC 1354/20)
    if (context.includes("NÃO FAZ JUS") || context.includes("REVISADO") || context.includes("INDEFERIDO") || status.includes("INDEFERIDO")) {
        ...
        O seu **processo, com a devida validação do Tempo de Contribuição, já retornou para a Unidade Escolar**. ${FECHO_CONSULTIVO}`;
    }

    // SITUAÇÃO: DEVOLVIDO / PENDÊNCIA / CORREÇÃO
    if (status.includes("DEVOLVIDO") || context.includes("DEVOLVIDO") || context.includes("CORREÇÃO") || context.includes("CORRECAO")) {
        return `Prezado(a) servidor(a), informamos que em **${dSaida || dEntrada}** o seu processo foi analisado por esta **Unidade Regional de Ensino** e foi identificada a necessidade de **correção ou complementação de documentos funcionais** para prosseguimento. 
        
        O processo retornou para a sua **Unidade Escolar** para que as providências necessárias sejam tomadas. Caso necessite de orientações detalhadas, por favor, procure a gerência de sua Unidade Escolar.`;
    }
    ...
    if (status.includes("FINALIZADO") || status.includes("CONCLUÍDO") || status.includes("CONCLUIDO") || context.includes("CONCLUIDO")) {
        ...
        // Sub-Caso: ABONO DE PERMANÊNCIA (Foco em ANEXOS e CÓPIAS)
        if (context.includes("ABONO")) {
            return `Prezado(a) servidor(a), informamos que o seu processo de VTC foi devidamente concluído por esta **Unidade Regional de Ensino** em **${dSaida || dEntrada}**. O seu **processo, com a devida validação do Tempo de Contribuição, já retornou para a Unidade Escolar** para ciência e registros fundamentais. 
            
            Ressaltamos que, para fins de pagamento do seu Abono de Permanência, é necessário providenciar os **ANEXOS e CÓPIAS** de documentações pertinentes e encaminhá-los para este setor. Para o prosseguimento quanto à concessão de aposentadoria, por favor, realize a solicitação diretamente junto ao setor **SEAPE** em sua Unidade Escolar.`;
        }
        
        // Sub-Caso: APOSENTADORIA / VTC PURO
        if (context.includes("APOSENTADORIA") || context.includes("VTC")) {
            return `Prezado(a) servidor(a), informamos que o seu processo de VTC foi devidamente concluído por esta **Unidade Regional de Ensino** em **${dSaida || dEntrada}**. O seu **processo, com a devida validação do Tempo de Contribuição, já retornou para a Unidade Escolar** para ciência e registros. 
            
            Orientamos que o(a) servidor(a) agora proceda com o trâmite necessário para solicitar a concessão de aposentadoria diretamente junto ao setor **SEAPE** em sua Unidade Escolar.`;
        }

        // Caso padrão de conclusão técnica
    ...
    // SITUAÇÃO: EM ANÁLISE / ANDAMENTO
    return `Prezado(a) servidor(a), informamos que seu processo deu entrada nesta **Unidade Regional de Ensino** em **${dEntrada}** e encontra-se atualmente em nossa **fila de análise técnica**. Nossa equipe está processando as solicitações seguindo rigorosamente a ordem cronológica de chegada para garantir a isonomia no atendimento. Recomendamos o acompanhamento periódico por este canal oficial.`;
}
...
function renderizarResultados(resultados, container) {
    ...
    resultados.forEach(processo => {
        ...
        // --- PALETA CONSULTATIVE ELEVATION (v11.6) ---
        let corBorda = "#003366"; 
        let EstiloBadge = "";
        let iconeBadge = "";
