...
// Versão 12.0.0 — Segurança Reforçada & LGPD
// ============================================================ 
...
async function consultarProcesso() {
    ...
    try {
        // MUDANÇA PARA POST: Mais seguro e evita cache de nomes completos
        const response = await fetch(API_PRODUCTION, {
            method: 'POST',
            headers: { 
                'Content-Type': 'application/json',
                'X-Turnstile-Token': turnstileToken 
            },
            body: JSON.stringify({ protocolo: pNumero })
        });

        const data = await response.json();

        if (!response.ok) {
            pResultado.innerHTML = `<div class="alert alert-danger border-0"><i class="bi bi-exclamation-triangle-fill me-2"></i>${data.erro || data.error || "Erro na consulta."}</div>`;
            return;
        }

        // A API nova retorna os dados dentro de "resultados"
...
function gerarMensagemHumanizada(processo) {
    ...
    const posicao = processo._posicaoFila ? `${processo._posicaoFila}º lugar` : "em processamento";
    ...
    if (context.includes("NÃO FAZ JUS") || status.includes("INDEFERIDO")) {
        return `Prezado(a) servidor(a), seu processo de VTC foi analisado em ${dSaida || dEntrada} e os requisitos ainda não foram integralmente preenchidos. O processo já retornou à Unidade Escolar. ${FECHO}`;
    }
    ...
    if (status.includes("FINALIZADO") || status.includes("CONCLUÍDO")) {
        return `Prezado(a) servidor(a), seu processo foi concluído em ${dSaida || dEntrada} e validado. Já retornou à Unidade Escolar para ciência e registros.`;
    }
    
    if (status.includes("ANALISE") || status.includes("ANDAMENTO")) {
        return `Prezado(a) servidor(a), seu processo deu entrada em ${dEntrada} e está na posição ${posicao}, aguardando análise cronológica.`;
    }
...
function renderizarResultados(resultados, container) {
    ...
    resultados.forEach(processo => {
        // O nome já vem mascarado da API, não precisamos de toUpperCase() aqui
        ...
        if (isEmAndamento && processo._posicaoFila) {
            ...
            dataEstimada.setDate(dataEstimada.getDate() + (processo._diasEstimados || 60));
            ...
            <div class="mt-3 p-3 bg-primary bg-opacity-10 border border-primary border-opacity-25 rounded-3 d-flex align-items-center justify-content-between shadow-sm">
                ...
                <div class="text-end border-start ps-3 border-primary border-opacity-25">
                    <span class="d-block small text-muted text-uppercase fw-bold" style="font-size:0.6rem;">Previsão</span>
                    <span class="fs-5 fw-bold text-primary">${dataEstimada.toLocaleDateString('pt-BR')}</span>
                </div>
            </div>`;
        }
                ...
                <div class="card mb-4 shadow-sm" style="border-radius: 12px; border-left: 8px solid #003366 !important; border: 1px solid #dee2e6; background: white;">
                    <div class="card-body p-4 position-relative">
                        ...
                        <div class="d-flex justify-content-between align-items-center pt-3 mt-3 border-top">
                            ...
                            <button class="btn btn-sm btn-light rounded-pill px-3 fw-bold" type="button" data-bs-toggle="collapse" data-bs-target="#collapse_${processo.id || Math.random()}">
                                Ver Detalhes <i class="bi bi-chevron-down ms-1"></i>
                            </button>
                        </div>

                        <div class="collapse mt-3" id="collapse_${processo.id || Math.random()}">
